...
void weather_request_update(int8_t set, int8_t cnt, char *type, int8_t period) {
	if (set == 1 || (set == 0 && strlen(type) > 0)) {
		...
		if (dict_write_uint8(iter, REQUEST_WEATHER_MSG_KEY, 1) != DICT_OK) return;
		if (dict_write_uint8(iter, REQUEST_WEATHER_GET_MSG_KEY, set) != DICT_OK) return;
		if (dict_write_cstring(iter, REQUEST_WEATHER_LOCATION_MSG_KEY, type) != DICT_OK) return;
		if (dict_write_uint8(iter, REQUEST_WEATHER_COUNT, cnt) != DICT_OK) return;
		if (dict_write_uint8(iter, 547, period) != DICT_OK) return;
...
void weather_set(TCW_Weather *weather, DictionaryIterator *iter) {
	...
	if (name) {
		strncpy(weather->name, name->value->cstring, sizeof(weather->name) - 1);
		weather->name[sizeof(weather->name) - 1] = '\0';
	}
	if (country) {
		strncpy(weather->country, country->value->cstring, sizeof(weather->country) - 1);
